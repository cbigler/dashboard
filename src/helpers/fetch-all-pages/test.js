...
describe('fetch-all-pages', function() {
  it('should fetch a bunch of pages of data', async function() {
    ...
    asyncMock.onCall(0).resolves({data: {next: 'https://api.density.io/v2/spaces?page=2', total: 18, results: [1, 2, 3, 4, 5]}});
    asyncMock.onCall(1).resolves({data: {next: 'https://api.density.io/v2/spaces?page=3', total: 18, results: [6, 7, 8, 9, 10]}});
    asyncMock.onCall(2).resolves({data: {next: 'https://api.density.io/v2/spaces?page=4', total: 18, results: [11, 12, 13, 14, 15]}});
    asyncMock.onCall(3).resolves({data: {next: null, total: 18, results: [16, 17, 18]}});
  ...
  it('should fetch a "batch counts" response, which is an object with arrays', async function() {
    ...
    asyncMock.onCall(1).resolves({ data: {
    ...
    asyncMock.onCall(2).resolves({ data: {
    ...
    asyncMock.onCall(3).resolves({ data: {
      next: null,
      total: 18,
      results: {
        foo: [16, 17, 18],
